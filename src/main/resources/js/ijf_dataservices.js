...
getGenericDataSourceSync:function(dataSource)
{
    ...
    $.ajax({
        async: false,
        type: 'GET',
        url: g_root + '/plugins/servlet/iftwl?ijfAction=proxyHttpRequest&url=' + encodeURIComponent(turl),
...
getNamedSource:function(dataSource,inFormKey,item,inField,inContainer)
{
   ...
   var turl = inField.dataReference;
    ...
    $.ajax(g_root + '/plugins/servlet/iftwl?ijfAction=proxyHttpRequest' + seqStr + '&url=' + encodeURIComponent(turl), {
        success: function(data) {
...
getRenderNamedSource:function(dataSource,inContainer)
{
    ...
    $.ajax(g_root + '/plugins/servlet/iftwl?ijfAction=proxyHttpRequest&url=' + encodeURIComponent(turl), {
        success: function(data) {
            //$('#main').html($(data).find('#main *'));
            ijfUtils.footLog("Successful data acquisition");
            //pause just a little for the initial rendering
