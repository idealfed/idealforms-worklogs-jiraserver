 ...
 isServerRuntime: function() {
    ...
    saveJiraFormSync:function(inJson,ijfAction){
		...
		jQuery.ajax({
				async: false,
				type: 'POST',
				url: g_root + '/plugins/servlet/iftwl',
				data: {
					jsonConfig: outJson,
					action: ijfAction
				},
...
{
    jQuery.ajax(g_root + '/plugins/servlet/iftwl?ijfAction=getConfig', {
        success: onSucess,
        error: onError
    });
},
		...
		if(thisT)
		{
			...
			if(!thisT.settings)
			{
			   var typeIndex = ijf.fw.CustomTypes.indexOf(thisT);
				...
				//load the settings...
               var fullTypeRaw = ijfUtils.jiraApiSync('GET',g_root + '/plugins/servlet/iftwl?ijfAction=getCustomType&customTypeId='+thisT.id, null);
