...
openForm:function()
{
			...
			if(thisF.testIssue)
			{
				var tUrl = g_root + '/plugins/servlet/iftwl?itemId='+thisF.testIssue+'&formId='+thisF.name + decorator;
			}
			else
			{
				var tUrl = g_root + '/plugins/servlet/iftwl?formId='+thisF.name + decorator;
			}
			window.open(tUrl);
},
renderReport_noforms:function(inContainerId)
{
	...
	else
	{
		...
		var fileDetailRaw = ijfUtils.jiraApiSync('GET',g_root + '/plugins/servlet/iftwl?ijfAction=getCustomType&customTypeId='+thisType.id, null);
	...
	cButtons.push({
		...
		handler: function(){
				...
				else
				{
					...
					if(!thisF.name){ ijfUtils.modalDialogMessage("Form Error","Sorry, undable to find the requested form: " + ijf.lists.itemId); return;}
					var tUrl = g_root + '/plugins/servlet/iftwl?debug=true&craft=true&itemId='+thisF.testIssue+'&formId='+thisF.name;
					window.open(tUrl);
				}
            }});
...
addEditCustomFileReference:function (inTypeId,isReportView)
{
    ...
    var fileDetailRaw = ijfUtils.jiraApiSync('GET',g_root + '/plugins/servlet/iftwl?ijfAction=getCustomType&customTypeId='+thisT.id, null);
							...
							var urlWin = new Ext.Window({
										...
										items: [{
											...
											xtype: "panel"},
											{
												...
												margin: '4 0 0 10',
												width: 900,
												value: window.location.origin + "/plugins/servlet/iftwl?mode=report&reportName="+thisT.name,
												allowBlank:false
											},{
											...
											xtype: "panel"},{
												...
												margin: '4 0 0 10',
												width: 900,
												value: window.location.origin + "/plugins/servlet/iftwl?mode=report&reportName="+thisT.name + "&issue="+fileDetail.key + "&jql="+fileDetail.jql + "&fields="+fileDetail.fields,
												allowBlank:false
											},{
