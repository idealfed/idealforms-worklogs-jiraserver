...
function init(inConfigVersion)
{
	...
	*/
    ...
    var configUrl = '/plugins/servlet/iftwl?ijfAction=getFormConfig&formId='+g_formId;
    if(g_formId=="") configUrl = '/plugins/servlet/iftwl?ijfAction=getConfig&version='+inConfigVersion;

    ijfUtils.footLog("Calling load configuration...");
//    jQuery.ajax(g_root + '/plugins/servlet/iftwl?ijfAction=getConfig&version='+inConfigVersion, {
//	  jQuery.ajax(g_root + '/plugins/servlet/iftwl?ijfAction=getFormConfig&formId='+g_formId, {
...
function processSetup(inContainerId)
{
    ...
    if ((ijf.main.itemId=='') && (window.g_formId==""))
    {
		...
		{
			if(ijfUtils.isServerRuntime())
				ijf.versionAdmin.render_BorderlayoutServer(inContainerId);
			else
