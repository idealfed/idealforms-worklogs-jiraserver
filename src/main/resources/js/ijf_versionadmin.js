...
ijf.versionAdmin = {
    ...
    getConfiguration:function(inId)
    {
		try
		{
            var thisConfig = ijfUtils.jiraApiSync("GET",g_IftRepo + '/plugins/servlet/iftwl?ijfAction=getConfig&version='+inId,null);
    ...
    getIftConfiguration:function(inId)
    {
		try
		{
            var thisConfig = ijfUtils.jiraApiSync("GET",g_IftRepo + '/ift/rest/product/'+g_IftProduct+'/version/'+inId,null);
    ...
    render_Borderlayout:function(inContainerId) {
		...
		var iftProductsRaw =  ijfUtils.jiraApiSync("GET",g_IftRepo +'/ift/rest/products',null);
		...
		var saveHistory = ijfUtils.jiraApiSync("GET","/plugins/servlet/iftwl","ijfAction=getVersions");
		...
		if(this.iftProduct)
		{
			var versions = ijfUtils.jiraApiSync("GET", g_IftRepo + '/ift/rest/product/' + this.iftProduct.id+ '/versions', null);
    ...
    render_BorderlayoutServer:function(inContainerId) {
        var colSpans = {};
        ijfUtils.setContent(inContainerId, 1, 1, colSpans, false, colSpans);
        ...
        this.iftProduct = {};
        this.iftProduct.name = "Ideal Worklogs for JIRA";
        this.iftProduct.description = "Ideal Worklogs provides two forms, \"Worklogs Report\" and \"My Worklogs\", that simplify worklog entry and tracking for both workers and managers.";

        if(ijf.fw.formSets.length>1)
        {
            //this is an installed producted.  Show versions and options...
            outHtml1="<div style='margin-bottom:5px;font-weight:bold;font-size:22pt'>Ideal Federal Technologies (IFT): " + this.iftProduct.name + "</div><br>Greetings and thank you for installing IFT Worklogs.  You are viewing the configuration options for your IFT product.";

            outHtml2 = "<div style='margin-top:5px;margin-bottom:5px;font-weight:bold;font-size:16pt'>About "+this.iftProduct.name+":</div><br>" +this.iftProduct.description;

            var pActionHtml = "";
            pActionHtml+="JIRA Issues can collect time worked logs when you enable JIRA Issues for time tracking using Settings->Issues->Time tracking->Enable. ";
            pActionHtml+="You must also set individual JIRA Project permissions to allow time entry and editing.  This is done the project's permission scheme.";
            pActionHtml+="<br><br>";
            pActionHtml+="Ideal Worklogs forms are accessed from a new button <b>\"Worklogs\"</b>, at the upper right, on the <a href='/issues/?jql=' target='_blank'>JIRA Search screen</a>";
            pActionHtml+=" and \"My Worklogs\" is accessed from within the Worklogs report form.";
            pActionHtml+="<br><br>";
            pActionHtml+="Both Ideal Worklog forms can be accessed directly from these links:";
            ...
            items: [
                {
                    ...
                    flex: 1,
                    width: "80%",
                    style: "height:auto;background:transparent; padding:5px 10px 10px 10px",
                    bodyStyle:  "overflow-wrap: break-word;height:300px;background:transparent; padding:5px 5px 5px 5px"},
                {
                    xtype: "panel",
                    html: outHtml2,
                    frame: false,
                    ...
                    width: "80%",
                    style: "height:auto;background:transparent;padding:5px 10px 10px 10px",
                    bodyStyle:  "overflow-wrap: break-word;height:auto;background:transparent; padding:5px 5px 5px 5px"},
                {
                    xtype: "panel",
                    id: "whatYouCanDoId",
                    html: outHtml3,
                    ...
                    width: "80%",
                    height: "auto",
                    style: "background:transparent; padding:5px 10px 10px 10px",
                    bodyStyle:  "overflow-wrap: break-word;background:transparent; padding:5px 5px 5px 5px"},
                {
                    xtype: "panel",
                    html: outHtml4,
                    frame: false,
